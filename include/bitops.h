...
// quite similar to bit shifts but here the bit that gets pushedd off the boundary will be reintroduced into the byte at the other end.
// 10110110 left rotated by 4 bits will be        01101011
// whereas left shifting by 4 bits will result in 00001011
static inline void leftrotbit(_Inout_ uint8_t* const bitstream, _In_ const size_t offset, _In_ const size_t n /* rotate n bits left */) {
    uint8_t leftbit = 0, rightbit = 0;
    if (!offset) return;
